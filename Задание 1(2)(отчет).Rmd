...
library('knitr')     # для форматирования таблиц
library('mlbench')
library('class')
library('car')
library('class')
...
Формируем случайную величину, обучающую и тестовую выборки.
...
Проанализируем работу байесовского классификатора.
...
Проделаем аналогичную работу для knn.
```{r knn}
#  k = 3
...
В результате было получено, что матрица неточностей на обучающей выборке и точность у обоих методов совпала. В дальнейшем будем работать с моделью, полученную методом knn.

Расчитаем TPR, SPC, PPV, NPV, FNR, FPR, FDR, MCC.
...
Полученные результаты говорят о высоком качестве разбиения. Это подтверждают и показатель точности Acc. 
